...
//            );
    ...
    public function manage_attendance($activity_id){
        ...
        //let's get what actually activity was being viewed.
        $this->data['activity_label'] = $this->activities_model->activity_edit($activities->activity); 
        $this->data['activity_details'] = $activities;
    ...
    /*
     * Function that will show lists of all available activities and other actions in activity listing page.
     */
    public function lists($action, $activity_id, $answer = ''){
        /* Breadcrumbs */
        $this->data['breadcrumb'] = $this->breadcrumbs->show();
        $this->data['lists'] = $this->activities_model->get_lists_activities();
        $this->data['activity_to_delete'] = '';
        $this->data['activity_label'] = '';
        $this->data['activity_box_header'] = 'Create';
        ...
        if( $this->input->post('action', TRUE) ){
            ...
            if( $action == 'create'){
                if( !empty($title) ){
                    $this->activities_model->activity_insert($title);
                    $this->session->set_flashdata('success', 'Successfully saved.');
                    $this->session->set_flashdata('error', '');
                }else{
                    $this->session->set_flashdata('success', '');
                    $this->session->set_flashdata('error', 'Activity Label field is required.');
                }
                
                redirect('admin/activities/lists', 'refresh');
            } 
            
            if( $action == 'update'){
                if( !empty($title) && (!empty($activity_id) && $activity_id > 0)){
                    $this->activities_model->activity_update($activity_id, $title);
        ...
        //Edit process...
        if( $action == 'edit' && (!empty($activity_id) && $activity_id > 0) ){            
            $this->data['activity_label'] = $this->activities_model->activity_edit($activity_id);
            $this->data['activity_box_header'] = 'Edit';
            $this->data['action'] = 'update';
        }
        
        //Delete process...
        if( $action == 'delete' && (!empty($activity_id) && $activity_id > 0) ){            
            if($answer == 'yes'){
                $this->activities_model->activity_delete($activity_id);
                $this->session->set_flashdata('success', 'Activity item was successfully deleted.');
                $this->session->set_flashdata('error', '');
                redirect('admin/activities/lists', 'refresh');
            }
            
            $this->data['activity_box_header'] = 'Delete';
            $this->data['action'] = 'delete';
            $this->data['activity_to_delete'] = $activity_id;
        }

        /* Load Template */
        $this->template->admin_render('admin/activities/lists', $this->data);
    }
